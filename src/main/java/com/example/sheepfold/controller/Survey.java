...
import com.example.sheepfold.service.ChooseService;
import com.example.sheepfold.service.DetailService;
import com.example.sheepfold.service.ModelService;
import com.example.sheepfold.service.QsnService;
import com.example.sheepfold.util.Api;
import com.example.sheepfold.util.String2Date;
...
//            return  api.returnJson(2,"此路不通");
    ...
    //用户第一题答题录入
    //modelId[]数组 但是只有一个model ID
    @RequestMapping(value="Index/survey/user/answer_list")
    @ResponseBody
    public Map<String,Object> userFirstAnswer(HttpServletRequest req) throws IOException{
//        String[] arr_modelId = req.getParameterValues("modelId[]");
          Map map=new HashMap();
//        if (arr_modelId.length==0)
//            return api.returnJson(3,"请选择模板答题");
//        String modelId=arr_modelId[0];
        ...
        modelList=modelService.findModelByTime(date);
        if(modelList==null||modelList.size()==0){//请注意这种判空方式
            return api.returnJson(2,"没有这个modelID 今日没答题");
        }
        ...
        if (!qsnList.get(0).getQsnType().equals(2)){//单选/多选
            ...
            map.put("qsnId",qsnList.get(0).getQsnId());
            map.put("bool_num",bool_num);
			map.put("optionListSize",optionList.size());
...
//            return  api.returnJson(2,"此路不通");
    ...
    //用户下一题
    @RequestMapping(value="Index/survey/user/nextqsn")
    @ResponseBody
    public Map<String,Object> userAnswerRecord(HttpServletRequest req) throws IOException{
        String modelId = req.getParameter("modelId");
        String str_orderNum = req.getParameter("orderNum");
        String qsnType = req.getParameter("qsn_type");
        String choose = req.getParameter("array");
        String qsnId=req.getParameter("qsnId");
        String psgId=req.getParameter("psgId");
        Map map=new HashMap();
        short orderNum=Short.parseShort(str_orderNum);
       // if(choose.length==0)
       //     return api.returnJson(3,"请答题");
...
//        }
        ...
        if (!qsnNext.getQsnType().equals(2)){//单选/多选
            ...
            map.put("qsnId",qsnNext.getQsnId());
            map.put("bool_num",bool_num);
            map.put("optionList",optionList1);
            map.put("optionListSize",optionList1.size());
//            for (int i = 0; i <optionList1.size() ; i++) {
//                map.put(i,optionList1.get(i));
//            }
            return  api.returnJson(1,"success",map);
...
//            return api.returnJson(1, "success", map);
//        }
        return api.returnJson(2, "fail1111", map);
    }

    //用户显示正确答案
    @RequestMapping(value="Index/survey/user/trueanswer")
    @ResponseBody
    public Map<String,Object> userAnswerTrue(HttpServletRequest req) throws IOException{
        Map map=new HashMap();
        map.put("one","D");
        map.put("two","A");
        map.put("three","D");
